...
// Update the weight of an existing validator, update the validators power index key
func (k Keeper) AddValidatorWeight(ctx sdk.Context, validator types.Validator,
	...
	validator = validator.AddWeight(weightToAdd)
...
// Update the weight of an existing validator, update the validators power index key
func (k Keeper) RemoveValidatorWeight(ctx sdk.Context,
	validator types.Validator, weightToRemove sdk.Int) types.Validator {

	k.DeleteValidatorByPowerIndex(ctx, validator)
	validator = validator.RemoveWeight(weightToRemove)
	k.SetValidator(ctx, validator)
	k.SetValidatorByPowerIndex(ctx, validator)
	return validator
}
